...
class RPNNoHeadBase(nn.Module):
    def __init__(self,
                 ...
                 upsample_strides=[.25, .5, 1],
                 num_upsample_filters=(128, 128, 128),
                 num_input_features=64,
                 num_anchor_per_loc=12,
                 encode_background_as_zeros=True,
                 use_direction_classifier=True,
                 use_groupnorm=False,
                 num_groups=32,
...
class RPNBase(RPNNoHeadBase):
    ...
    def forward(self, x):
        ...
        #################################################################################################333
        # mayank modification for dict removal
        ...
        ret_dict = [box_preds, cls_preds, dir_cls_preds]
        return ret_dict
...
model = RPNBase()
# model=RPNNoHeadBase()
# model= get_rpn_class('rpn')


# x = torch.randn(1, 64,400,400, device='cuda')
x = torch.randn(1, 64, 400, 400)
# cd = torch.randn(8500, 4, device='cuda')

cn = model(x=x)

model.load_state_dict(torch.load('working_rpn.pth'))

# print(model.weight)
# print(state_dict.keys())
torch.save(model.state_dict(), "myrpn1.pth")
torch.onnx.export(model, x, "mayank_rpn.onnx")
