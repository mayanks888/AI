...
def conv_loc_to_velo_pt(center, ry):
    ...
    # mlab.points3d(corners_3d[0][0], corners_3d[0][1], corners_3d[0][2], 0.5,figure=fig)
    # return fig
    # mlab.show()
...
for example in example_dict:
    ...
    for scores in example['scores']:
        if scores > 0.50:
            print(scores)
            dt_corner = example['loc'][counter]
            angle = example['angle'][counter]
            # fig=draw_gt_boxes3d(dt_corner, fig)
            centre_val = conv_loc_to_velo_pt(dt_corner, angle)
            # fig=conv_loc_to_velo_pt(dt_corner,angle,fig)
            # x=centre_val[0][0]
            x, y, z = centre_val[0][2], centre_val[0][1], centre_val[0][0]
            print(x, y, z)
            # mlab.points3d(centre_val[0][2], centre_val[0][1], centre_val[0][0], scale_factor=1.0,figure=fig)
            mlab.points3d(x, y, z, scale_factor=1.0, figure=fig)
        counter += 1
        ###############################################
    mlab.show()
