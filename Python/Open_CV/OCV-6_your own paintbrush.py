...
while (True):
    ...
    if ch & 0XFF == ord('q'):
        ...
        circle_color = (
            255, 0, 0)  # this is to change the color (if key 'b' is pressed then change circle color to blue)
    elif ch & 0XFF == ord('r'):
        circle_color = (
            0, 0, 255)  # this is to change the color (if key 'r' is pressed then change circle color to red)
