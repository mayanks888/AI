...
print(np.sqrt(arr2))
######################################3333
# adding data into dimension in numoy
def swap_cols(arr, frm, to):
    arr[:,:,[frm, to]] = arr[:,:,[to, frm]]
    return arr

# swap_cols(my_array, 0, 1)
pred1 = np.random.randn(1, 7131, 6)
# temp = np.random.randn(1, 7131, 1)
temp1 = np.zeros((1, 7131, 80))
kp3=np.concatenate((pred1,temp1),axis=2)
kp1=swap_cols(kp3, 5, -1)
y = np.expand_dims(pred1, axis=1)
pred2 = np.random.randn(1, 7130, 85)
temp2 = np.zeros((1, 7130, 1))
kp2=np.concatenate((pred2,temp2),axis=2)
# result = np.repeat(pred1[np.newaxis, ...], 10, axis=0)
# print(result.shape)
kp=np.concatenate((kp1,kp2),axis=1)
1
a = np.array([[1, 2], [3, 4]])
b = np.array([[5, 6]])
