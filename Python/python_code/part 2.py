...
myseries.isnull()
print
"the null count is", myseries.isnull().sum()
print
myseries.notnull()
print
"the not null count is", myseries.notnull().sum()
...
print
data['Survived'].notnull().sum()
print
data['Survived'].count()
# lets replace NAN with some value eg pizza
new_data = data["Age"].replace(np.nan, 'pizza')
print
new_data.head()  # here we have removed all the nan  with male
print
new_data.isnull().sum()
print
new_data
