...
print
data.drop(labels=data.columns[[0, 4]], axis=1).head()
...
print
data_embarked.drop_duplicates(subset=['Embarked']).head()

groupby in dataframe
print
data.groupby("Age").mean()  # so we will get the group of only numerical data
print
data.groupby(["Age", "Sex"])['Survived', 'Pclass'].mean()  # groupby only specific column

print
data['Name'].upper()  # not working

for name_data in data['Name']:
    print
    name_data.upper()  # upper class name
