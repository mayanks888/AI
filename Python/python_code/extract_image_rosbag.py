...
# Requried flags.
gflags.DEFINE_string('input_bag', '/home/mayanksati/Desktop/dataset.bag', 'Input bag path.')
...
def extract_images(bag, dst_dir, args):
    ...
    for _, msg, t in bag.read_messages(topics=args.topic):
        ...
        # Save image.
        seq += 1
        strs = str("{:05d}".format(seq))
        msg.encoding = 'yuv422'
        img = bridge.imgmsg_to_cv2(msg, 'yuv422')
        img = cv2.cvtColor(img, cv2.COLOR_YUV2BGR_YUYV)
        img_file = os.path.join(dst_dir, '{}.jpg'.format(strs + '_' + args.topic.split('/')[5]))
        # img_file = os.path.join(dst_dir, '{}.jpg'.format(str(seq)+'_' + args.topic.split('/')[5]))
