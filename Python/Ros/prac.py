...
def velo_callback(msg):
    ...
    if len(arr_bbox.boxes) is not 0:
        for i in range(0, len(arr_bbox.boxes)):
            print("[+] [x,y,z,dx,dy,dz] : {}, {}, {}, {}, {}, {}".format(arr_bbox.boxes[i].pose.position.x,
                                                                         arr_bbox.boxes[i].pose.position.y,
                                                                         arr_bbox.boxes[i].pose.position.z,
                                                                         arr_bbox.boxes[i].dimensions.x,
                                                                         arr_bbox.boxes[i].dimensions.y,
                                                                         arr_bbox.boxes[i].dimensions.z))
...
if __name__ == '__main__':
    ...
    sub_ = rospy.Subscriber("velodyne_points", PointCloud2, queue_size=1)

    pub_velo = rospy.Publisher("velodyne_points_modified", PointCloud2, queue_size=1)
    pub_arr_bbox = rospy.Publisher("voxelnet_arr_bbox", BoundingBoxArray, queue_size=10)
