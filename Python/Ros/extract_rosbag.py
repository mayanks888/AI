...
# filepath='/home/mayanksati/Documents/point_clouds/kitt_rosbag/2011_09_30_drive_18.bag'
filepath = '/home/mayank_sati/Documents/datsets/Rosbag_files/78_gb_us_demo_route.bag'
filepath = '/home/mayank_sati/Desktop/new_bag.bag'
# Requried flags.
...
gflags.DEFINE_string('topic', '/apollo/sensor/camera/traffic/image_short', 'Source topic.')
...
def extract_meta_info(bag):
    ...
    static_info = next((msg for _, msg, _ in bag.read_messages(topics=[kStaticInfoTopic])), None)
...
def extract_images(bag, dst_dir, args):
    ...
    for _, msg, t in bag.read_messages(topics=args.topic):
        ...
        img_file = os.path.join(dst_dir, '{}.jpg'.format(str(seq) + args.topic.split('/')[5]))
        # img_file = '{}.jpg'.format(str(seq)+args.topic.split('/')[5])
        cv2.imwrite(img_file, img)
        print(img_file)
        # glog.info('#{}: header.seq={}, header.stamp={}, saved as {}'.format(seq, msg.header.seq, cur_time_sec, img_file))
