...
def normalized_columns_initializer(weights, std=1.0):
    ...
    out *= std / torch.sqrt(out.pow(2).sum(0).expand_as(out))  # thanks to this initialization, we have var(out) = std^2
    # out *= std / torch.sqrt(out.pow(2).sum(1).expand(256,4))  # thanks to this initialization, we have var(out) = std^2
    # dat=torch.sqrt(out.pow(2).sum(1))
    # dat=(out.pow(2).sum(0))
    # cool=dat.expand_as(out)
    # ct=std / cool
    # out *= ct
    # out *= std / torch.sqrt(out.pow(2).sum(1))
    # out1=out.expand_as(out) # thanks to this initialization, we have var(out) = std^2
    return out
...
def weights_init(m):
    ...
    if classname.find('Conv') != -1:  # if the connection is a convolution
        ...
        m.weight.data.uniform_(-w_bound,
                               w_bound)  # generating some random weights of order inversely proportional to the size of the tensor of weights
...
class ActorCritic(torch.nn.Module):
    ...
    def __init__(self, num_inputs, action_space):
        ...
        self.lstm = nn.LSTMCell(32 * 3 * 3,
                                256)  # making an LSTM (Long Short Term Memory) to learn the temporal properties of the input - we obtain a big encoded vector S of size 256 that encodes an event of the game
        ...
        self.actor_linear.weight.data = normalized_columns_initializer(self.actor_linear.weight.data,
                                                                       0.01)  # setting the standard deviation of the actor tensor of weights to 0.01
        ...
        self.critic_linear.weight.data = normalized_columns_initializer(self.critic_linear.weight.data,
                                                                        1.0)  # setting the standard deviation of the critic tensor of weights to 0.01
    ...
    def forward(self, inputs):
        ...
        x = F.elu(
            self.conv1(inputs))  # forward propagating the signal from the input images to the 1st convolutional layer
        x = F.elu(self.conv2(
            x))  # forward propagating the signal from the 1st convolutional layer to the 2nd convolutional layer
        x = F.elu(self.conv3(
            x))  # forward propagating the signal from the 2nd convolutional layer to the 3rd convolutional layer
        x = F.elu(self.conv4(
            x))  # forward propagating the signal from the 3rd convolutional layer to the 4th convolutional layer
        x = x.view(-1, 32 * 3 * 3)  # flattening the last convolutional layer into this 1D vector x
        hx, cx = self.lstm(x, (
            hx,
            cx))  # the LSTM takes as input x and the old hidden & cell states and ouputs the new hidden & cell states
        ...
        return self.critic_linear(x), self.actor_linear(x), (hx,
                                                             cx)  # returning the output of the critic (V(S)), the output of the actor (Q(S,A)), and the new hidden & cell states ((hx, cx))

# name=ActorCritic(4,3)
