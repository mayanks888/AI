...
class Object_detect():
    ...
    def Laod_model_parameter(self):
        ...
        PATH_TO_CKPT = os.path.join(CWD_PATH, self.MODEL_NAME, 'frozen_inference_graph_20000.pb')
    ...
    def bbox_size_change(self, data_label, new_hight, new_width):
        width = data_label[5]
        height = data_label[4]
        ...
        for root, _, filenames in os.walk(input_folder):
            ...
            for filename in filenames:
                try:
                    ...
                    time_start = time.time()
                    resize_image_ = tf.image.resize_images(image, size=(resize_height, resize_width),
                                                           align_corners=False, preserve_aspect_ratio=False)
                    # image_2 = tf.image.resize_image_with_crop_or_pad(image, target_height=resize_height, target_width=resize_width)
                    new_sess = tf.Session()
                    resize_image_2 = new_sess.run(resize_image_)
                    image_expanded = np.expand_dims(resize_image_2, axis=0)
                    # print(res)
                    ###########################################################
                    ...
                    (boxes, scores, classes, num) = self.sess.run(
                        ...
                        feed_dict={self.image_tensor: image_expanded}, options=options, run_metadata=run_metadata)

                    # fetched_timeline = timeline.Timeline(run_metadata.step_stats)
                    # chrome_trace = fetched_timeline.generate_chrome_trace_format()
                    time_end = time.time()
                    # with open('timeline_01.json', 'w') as f:
                    # with open("/home/ubuntu/Documents/traffic_light/json/"+filename+".json", 'w') as f:
                    #     f.write(chrome_trace)
                    # print (self.num_detections,'\n')
                    # print(scores[scores>.25])
                    # Draw the results of the detection (aka 'visulaize the results')
                    #########################################################################################
                    # here I'll try to change the bounding box rescaling'
                    ...
                    #
                    # dat_return = self.bbox_size_change(data_label, new_hight=image.shape[0], new_width=image.shape[1])
                    ...
                    res = (str(image_expanded.shape[1]) + "*" + str(image_expanded.shape[2]))
                    cv2.putText(image, res, (90, 70), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 0, 255),
                                lineType=cv2.LINE_AA)
                    cv2.imwrite(output_path, image)
                    # cv2.imwrite(output_path, resize_image_2)
...
def parse_args():
    ...
    file_path = "/home/mayanksati/PycharmProjects/Data_Science_new/Task/object_detector_2/training/evaluate_square_traffic_light"
...
# MODEL_NAME = 'training/ssd_resnet_50_fpn/single/newly_train__jan'
