...
class Object_detect():
    ...
    def Laod_model_parameter(self):
        ...
        PATH_TO_CKPT = os.path.join(CWD_PATH, self.MODEL_NAME, 'frozen_inference_graph_100000.pb')
        ...
        for root, _, filenames in os.walk(input_folder):
            ...
            for filename in filenames:
                try:
                    ...
                    #########################################################
                    time_start = time.time()
                    # image_2 = tf.image.resize_image_with_crop_or_pad(image, target_height=300, target_width=300)
                    image_2 = tf.image.resize_images(image, size=(300, 300),
                                                     align_corners=False, preserve_aspect_ratio=False)
                    new_sess = tf.Session()
                    image = new_sess.run(image_2)
                    image_expanded = np.expand_dims(image, axis=0)
                    # print(res)
                    ###########################################################
...
def parse_args():
    ...
    # file_path="/home/ubuntu/PycharmProjects/ob_2/training/evaluate_square_traffic_light"
