...
#################################################3333
#2nd csv
csv_path='/home/mayank_s/codebase/others/yolo/yolov4_custom/yolov4_nuscene_prediction_all_folder.csv'
# csv_path='/home/mayank_s/codebase/others/yolo/yolov4_custom/yolov4_nuscene_prediction_new.csv'

data_2 = pd.read_csv(csv_path)
new_val = data_2.iloc[:].values
for i,values in enumerate(new_val):
    print("file :  {fl}".format( fl=i), '\n')

    doomed_called = 0
    # if i >2000:
    #     break
    filename, width, height,obj_name ,xmin, ymin, xmax, ymax, score, crop_width, crop_height=values
    # filename, width, height,obj_name ,xmin, ymin, xmax, ymax, score=values
...
#######################################################
    ...
    while(found_flag==False):
        ...
        for da,ad in zip(m.index,m):
            ...
            if (da[0]==crop_width and da[1]==crop_height):
                ...
                for dummy in range(len(ad)):
                    ...
                    got_replacement+=1
                    print("got replacement for file" , got_replacement)
                    found_flag=True
                    break
            if found_flag==True:
                break
        if found_flag==False:
            ...
            print("doomed_called",doomed_called)
...
df.to_csv('nuscene_manipulate_all_mini_2.csv', index=False)
