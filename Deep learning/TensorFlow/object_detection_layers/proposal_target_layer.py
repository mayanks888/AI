...
def proposal_target_layer_py(rpn_rois, gt_boxes, num_classes):
    ...
    all_rois = np.vstack(
        (all_rois, np.hstack((zeros, gt_boxes[:, :-1]))))  # append gt_boxes at the last row of all_rois
    ...
    labels, rois, bbox_targets, bbox_inside_weights = sample_rois(all_rois, gt_boxes, fg_rois_per_image, rois_per_image,
                                                                  num_classes)
