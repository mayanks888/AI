...
def anchor_target_layer_python(rpn_cls_score, gt_boxes, im_dims, feat_strides, anchor_scales):
    ...
    argmax_overlaps = overlaps.argmax(
        axis=1)  # for each anchor( for each row), find its max overlap gt box index among all the gt_boxes
    max_overlaps = overlaps[np.arange(
        len(inds_inside)), argmax_overlaps]  # [ Finds_inside, 1], return the max_overlap area for each anchor
    gt_argmax_overlaps = overlaps.argmax(
        axis=0)  # for each gt_box, find its max overlap against all the anchors, it return the anchor index
    gt_max_overlaps = overlaps[
        gt_argmax_overlaps, np.arange(overlaps.shape[1])]  # for each ground truth, return it maximum overlap
    ...
    gt_argmax_overlaps = np.where(overlaps == gt_max_overlaps)[
        0]  # return all the anchors index which has maximun overlap
