...
class RegionProposalNetwork(object):
    ...
    def __init__(self, feature_vector, ground_truth, im_dims, anchor_scale, Mode):
        ...
        self.weights = {
            'w_rpn_conv1': tf.Variable(
                tf.random_normal([self.RPN_KERNEL_SIZE, self.RPN_KERNEL_SIZE, 512, self.RPN_OUTPUT_CHANNEL],
                                 stddev=0.01)),
    ...
    def build_rpn(self):
        ...
        with tf.variable_scope('cls'):
            self.rpn_cls_score = tf.nn.conv2d(feat, self.weights['w_rpn_cls_score'], strides=[1, 1, 1, 1],
                                              padding='SAME')

        # bounding-box prediction 
        with tf.variable_scope('reg'):
            self.rpn_reg_pred = tf.nn.conv2d(feat, self.weights['w_rpn_bbox_pred'], strides=[1, 1, 1, 1],
                                             padding='SAME')
        ...
        with tf.variable_scope('anchor'):
            if self.mode == 'train':
                self.rpn_labels, self.rpn_bbox_targets, self.rpn_bbox_inside_weights, self.rpn_bbox_outside_weights = \
                    anchor_target_layer.anchor_target_layer(self.rpn_cls_score, self.ground_truth, self.im_dims,
                                                            self.feat_stride, self.anchor_scale)
