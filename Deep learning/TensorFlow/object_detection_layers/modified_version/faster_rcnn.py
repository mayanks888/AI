...
class FasterRCNN(object):
    ...
    def faster_rcnn(self, x, gt_bbox, Mode='train'):
        ...
        self.rpn[Mode] = region_proposal_network.RegionProposalNetwork(features, self.gt_bbox['train'],
                                                                       self.im_dims['train'], self.anchor_scale, Mode)
        # run ROI Pooling
        self.roi[Mode] = roi_proposal_layer.roi_proposal_layer(self.rpn[Mode], self.gt_bbox['train'],
                                                               self.im_dims['train'], Mode)
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            for step in range(0, self.num_steps):
                ...
                for start in range(0, self.datahandler.num() - self.datahandler.num() % self.batch_size,
                                   self.batch_size):
                    ...
                    rpn_input_feat, rpn_cls_score, rpn_label, rpn_cls_loss, rpn_bbox_pred, roi_blob, pooled_features = \
                        sess.run([self.init_feature, self.cost, self.rpn_label, self.rpn_cls_loss, self.rpn_bbox_pred,
                                  self.roi_blob, self.pooled_features]
                                 , feed_dict={self.X['train']: data, self.gt_bbox['train']: label,
                                              self.im_dims['train']: im_dim})
