...
base_model = tf.keras.applications.vgg16.VGG16(weights="imagenet", include_top=False,
                                               input_tensor=x_image)  # loading vgg16  model trained n imagenet datasets
...
rpn_spc_layer = tf.keras.layers.Conv2D(filters=512, kernel_size=(1, 1), strides=(1, 1), activation='relu',
                                       padding='same', name='rpn_conv_1')(last_layer)
rpn_cls_score_lr = tf.keras.layers.Conv2D(filters=18, kernel_size=(1, 1), strides=(1, 1), activation='relu',
                                          padding='same', name='rpn_class_score')(rpn_spc_layer)
rpn_bb_reg_lr = tf.keras.layers.Conv2D(filters=36, kernel_size=(1, 1), strides=(1, 1), activation='relu',
                                       padding='same', name='rpn_bbox')(rpn_spc_layer)
...
rpn_labels, rpn_bbox_targets, rpn_bbox_inside_weights, rpn_bbox_outside_weights = anchor_target_layer.anchor_target_layer(
    rpn_cls_score_lr, gt_bbox, im_dims, feat_stride, anchor_scale)

rpn_box_loss = region_proposal_network2.rpn_bbox_loss(rpn_bb_reg_lr, rpn_bbox_targets, rpn_bbox_inside_weights,
...
for loop in range(epochs):
    ...
    for (image_input, gt_box, image_dim) in ds.batch_iterator(batch_size=1,
        ...
        _, loss, target = sess.run([train, total_loss, rpn_bbox_targets],
                                   feed_dict={input_x: image_input, gt_bbox: gt_box, im_dimsal: image_dim})
