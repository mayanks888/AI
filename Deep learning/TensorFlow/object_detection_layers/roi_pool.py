...
def roi_pool(feature_map, rois, im_dims):
    ...
    with tf.variable_scope('roi_pool'):
        ...
        box_ind = tf.cast(rois[:, 0],
                          dtype=tf.int32)  # 1-D tensor of shape [num_boxes]. The value of box_ind[i] specified the image that the i-th box refers to.
        ...
        boxes = tf.stack([boxes[:, 1], boxes[:, 0], boxes[:, 3], boxes[:, 2]],
                         axis=1)  # y1, x1, y2, x2 -> to fit tf.image.crop_and_resize
