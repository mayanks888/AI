...
def normal_full_layer(input_layer, output_layer_size, var_name):
    input_size = int(
        input_layer.get_shape()[1])  # this is used to get the column no from shape of fully connected layers
    W = create_weight([input_size, output_layer_size], var_name=(var_name + "_weight"))
    b = create_bias([output_layer_size], var_name=(
            var_name + "_bias"))  # remember bias are always equal to the no of output layer in fully connected layer
...
full_layer_one = tf.nn.relu(
    normal_full_layer(convo_2_flat, 1024, var_name="full_1"))  # now the output will be 1 row and 1024 column

# NOTE THE PLACEHOLDER HERE!
hold_prob = tf.placeholder(
    tf.float32)  # this made to pass the user defined value for dropout probabilty you could have also used contant value
full_one_dropout = tf.nn.dropout(full_layer_one, keep_prob=hold_prob)

# last layer for classifications
y_pred = normal_full_layer(full_one_dropout, 10,
                           var_name="full_last")  # remember I have not used softmax or relu in the output of late layer
...
sum_loss_per_row = (
    -tf.reduce_sum(entropy_loss_per_row, axis=1))  # "-" you have to check(axis=1 since I was row wise sum)
...
'''for loop in range(epochs):
    ...
    save_path = saver.save(sess, "../../../../Datasets/MNIST_data/model_weight/model.ckpt")'''
