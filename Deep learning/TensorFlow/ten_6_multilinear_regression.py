...
y = tf.placeholder(dtype=tf.float32, name="y",
                   shape=[None, 1])  # (this will generae a placeholder of 506 rows and 1 column)

# declaring variables
w = tf.Variable(
    initial_value=tf.random_normal([m_coumn, 1]))  # y=wx+b(check rule for matrix multiplication before assigning values
...
for i in range(epoch):
    ...
    print('epoch{0}:loss{1}'.format(i,
                                    total_loss / n_sample))  # this is good method to print with description check this out
    final_w = sess.run(w)
    print(final_w)
