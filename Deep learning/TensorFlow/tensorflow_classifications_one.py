...
model_dnn = tf.estimator.DNNClassifier(hidden_units=[10, 10, 10], feature_columns=feat_column_2,
                                       n_classes=2)  # hidden unit[10,10,10] means 3 hidden layer with 10 neuron each
model_dnn.train(input_function, steps=1000)
evaluation = tf.evaluation = tf.estimator.inputs.pandas_input_fn(x=X_test, y=y_test, batch_size=10, num_epochs=1,
                                                                 shuffle=False)
