...
def mymultilayer_perceptron(my_input_matrix):  # one function will take care of all weight and bias thing
    ful_con_layer_1 = tf.contrib.layers.fully_connected(my_input_matrix, hidden_layer_neuron, activation_fn=tf.nn.relu,
                                                        scope='fc1')
    out_ful_conected_layer = tf.contrib.layers.fully_connected(ful_con_layer_1, output, activation_fn=tf.sigmoid,
                                                               scope='ol')
...
for i in range(epochs):
    ...
    my_prediction_train = tf.equal(tf.argmax(my_softmax_val, 1), tf.argmax(batch_y,
                                                                           1))  # comparing max index of softmax output and true y output matrix
...
my_prediction = tf.equal(tf.argmax(y_hat, 1),
                         tf.argmax(output_matrix, 1))  # comparing max index of softmax output and true y output matrix
# cast convert true to 1 an false to 0
accuray = tf.reduce_mean(tf.cast(my_prediction, tf.float32))
prediction_val, final_acuracy = (
    sess.run([my_prediction, accuray], feed_dict={input_matrix: mnist.test.images, output_matrix: mnist.test.labels}))
