...
learning_rate = .03
drop_out = 0.85
input_matrix = tf.placeholder(dtype=tf.float32, shape=[None, total_pixel_input],
                              name='input_matrix')  # (no of rows*728)
...
def create_conv2d(input_matrix_converted, filter_weight, bias, strides=1):
    val = tf.nn.conv2d(input=input_matrix_converted, filter=filter_weight, strides=[1, strides, strides, 1],
                       padding='SAME')
...
for i in range(epochs):
    ...
    # print (actual_weight)
    layer_output, mycross_entropy, my_softmax_val, all_cross_ent, _ = sess.run(
        [pred_value, loss, softmax_output, total_cross_entropy, optimizer],
