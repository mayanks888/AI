...
def train_model(data, model, hyper_params, model_names):
    ...
    # history = model.fit(x_train, y_train, batch_size=batch_size, epochs=epochs, validation_data=(x_valid, y_valid), shuffle=True, verbose =1, callbacks =[checkpoint, tensorboard]).history
    # end_time = time.time()
