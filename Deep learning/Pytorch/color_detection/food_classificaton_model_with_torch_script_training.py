...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(57 * 57 * 4, 16)
        # self.fc1 = nn.Linear(800, 1000)
        # self.fc2 = nn.Linear(1000, 64)
        self.fc3 = nn.Linear(16, 19)
...
def main():
    # Training settings
    training_dir = "/home/mayank_sati/codebase/python/camera/tensorflow/Food_DB_Demo (1)/ECUSTFED_DB"
...
########################################33
    ...
    traced_script_module.save("food_classification.pt")
    loaded_model = torch.jit.load("food_classification.pt")
    model = loaded_model.train(True)
    ########################################################
    ...
    if (args.save_model):
        torch.save(model.state_dict(), "food_classify.pt")
