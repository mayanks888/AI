...
class ProposalTargetCreator(object):
    ...
    def make_proposal_target(self, roi, gt_bbox, gt_bbox_label):
        ...
        roi = np.concatenate((roi, gt_bbox),
                             axis=0)  # nice techinqe to use ground t bounding box for your proper training
        ...
        neg_index = \
            np.where((max_iou_for_roi >= self.neg_iou_thresh_low) & (max_iou_for_roi < self.neg_iou_thresh_high))[0]
        ...
        target_delta_for_sample_roi = (target_delta_for_sample_roi - np.array([0., 0., 0., 0.])) / np.array(
            [0.1, 0.1, 0.2, 0.2])
