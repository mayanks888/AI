...
def register_rpn(cls, name=None):
    ...
    if name is None:
        name = cls.__name__
    assert name not in REGISTERED_RPN_CLASSES, "exist class: {REGISTERED_RPN_CLASSES}"
...
def get_rpn_class(name):
    global REGISTERED_RPN_CLASSES
    assert name in REGISTERED_RPN_CLASSES, "available class: {REGISTERED_RPN_CLASSES}"
...
# this  was first called from rpn script
